...
> Target repo: `adityahimaone/hermes-webui-personal` (fork of `nesquena/hermes-webui`)
...
| Doc | Purpose |
|---|---|
| [`docs/01-architecture-design.md`](docs/01-architecture-design.md) | Target Go architecture, package layout, tech choices, how Go talks to the existing Python/Hermes agent core |
| [`docs/02-api-parity-mapping.md`](docs/02-api-parity-mapping.md) | Every existing Python endpoint/behavior mapped to its Go equivalent — the literal 1:1 contract |
| [`docs/03-mvp-scope.md`](docs/03-mvp-scope.md) | What "MVP" means here, cut lines, definition of done |
| [`docs/04-task-breakdown.md`](docs/04-task-breakdown.md) | Ordered, checkbox-level task list per phase |
| [`docs/05-migration-strategy.md`](docs/05-migration-strategy.md) | How to run Go + Python side-by-side safely (strangler-fig proxy), rollback plan |
| [`docs/06-testing-and-parity.md`](docs/06-testing-and-parity.md) | How we prove 1:1 parity before flipping traffic, regression gates |
| [`docs/07-future-vite-frontend.md`](docs/07-future-vite-frontend.md) | Deferred: frontend rewrite to Vite, only after backend is 100% Go |
| [`docs/08-kanban-board.md`](docs/08-kanban-board.md) | Kanban/task-dispatch board — data model, dispatcher semantics, its own phase |
| [`docs/09-phase4b-unblock.md`](docs/09-phase4b-unblock.md) | **Live fix**: unblocks Phase 4b's gRPC transport — working `transport.go`/`grpcclient.go` code, mock-server tests, and a Python gRPC shim skeleton |
