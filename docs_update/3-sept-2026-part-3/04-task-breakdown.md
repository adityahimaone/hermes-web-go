...
- [ ] Remove proxy fallback for chat routes — **blocked on `docs/10-chat-parity-fixes.md`** (4 known
      chat-parity gaps found via `chat_api_parity.md`: a critical HTTP-transport SSE parser bug that
      silently breaks the Phase 4b `auto`-mode fallback safety net, a duplicate completion event,
      and dropped partial answers on agent errors). Fix and test all of §10 before this cutover —
      budget extra soak time regardless; this is the highest-risk cutover in the whole plan.
